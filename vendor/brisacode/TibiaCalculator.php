...
public function findTotalBalance( $return = false ){

    $partyData = $this->getPartyData();

    $totalBalance = trim($this->getStringBetween($partyData, "Balance: ", " " ));    

    if($return){return $this->setTotalBalance($totalBalance);}
}
...
public function findNumberOfPlayers( $return = false ){
    ...
    if($return){return  (int)$numberOfPlayers -1;}
}



public function findPlayersData( $return = false ){     
   ...
   for ($i=0; $i < $this->getNumberOfPlayers(); $i++) {     
    ...
    if($return){return $this->getPlayersData();}
...
public function generateExtractedSessionData(){
    ...
    $playersData = $this->findPlayersData( true );
